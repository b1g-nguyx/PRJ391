...
<html>
    <head>
                        ...
                        <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasNavbar" aria-labelledby="offcanvasNavbarLabel">
                            ...
                            <div class="offcanvas-body">
                                <ul class="navbar-nav justify-content-center align-items-center flex-grow-1 pe-3">
                                    ...
                                    <li class="nav-item">
                                        <a class="nav-link active mx-lg-2" href="#">Products</a>
                                    </li>
                                    <li>
        ...
        <div class="row p-5 ">
            <div class="col-2">
                <div class="pt-5">
                    <div class="list-group text-uppercase shadow rounded-2">
                        <a class="p-3 m-0 fw-semibold text-bg-dark rounded btn" href="product.jsp">Product</a>
                        <c:forEach var="i" items="${pc.all}">
                            <a  href="/Project_of_assignment/listP?pc=${i.id}&&index=1" class="list-group-item list-group-item-action px-3 fw-semibold ${i.id == pcc.id ? "text-bg-secondary":""}">${i.name}</a>
                        </c:forEach>
                    </div>
                </div>
                        ...
                        <div class="row  d-flex justify-content-xxl-evenly" >
                            ...
                            <c:if test="${all == null}">
                                <c:if test="${requestScope.listP == null}">
                                    <c:forEach var="i" items="${pc.all}">
                                        <div class=" d-flex justify-content-between">
                                            <span class="text-uppercase fw-semibold fs-2">${i.name}</span>
                                            <a href="listP?pc=${i.id}&&index=1" >see all</a> 
                                        </div> 
                                        <hr/>
                                        <c:set var="count" value="1"/>
                                        <c:forEach var="y" items="${p.all}">
                                            <c:if test="${i.id == y.category_id}">
                                                <c:if test="${count < 4}">
                                                    <div class="col-auto ps-2 pb-3">
                                                        <div class="card shadow" style="width: 18rem;">
                                                            <a href="/Project_of_assignment/detail?p=${y.id}&&pc=${i.name}" style="" class="disabled text-dark text-decoration-none link-underline-opacity-0" id="moreinformation">
                                                                <img src="assets/img/products/${y.image}" class="shadow card-img-top img-thumbnail" alt="${y.image}">
                                                                <div class="card-body">
                                                                    <h5 class="card-title">${y.name}</h5>
                                                                    <p class="card-text">${y.descriotion}</p>
                                                                    <p class="ps-2 card-text text-secondary">${y.price} VND</p>
                            ...
                            <c:if test="${requestScope.listP != null}">
                                <div class=" d-flex justify-content-between">
                                    <span class="text-uppercase fs-2">${pcc.name}</span>
                                </div> 
                                ...
                                <nav aria-label="Page navigation example">
                                    <ul class="pagination justify-content-center pb-3">
                                        <c:if test="${tag-1>0}">
                                            <li class="page-item">
                                                <a class="page-link" href="/Project_of_assignment/listP?pc=${pcc.id}&&index=${tag-1}" >Previous</a>
                                            </li>
                                        </c:if>
                                        <c:forEach begin="1" end="${requestScope.endP}" var="i">
                                            <li class="page-item ${tag == i ? "active":""}"><a class="page-link" href="/Project_of_assignment/listP?pc=${pcc.id}&&index=${i}">${i}</a></li>
                                            </c:forEach>
                                            <c:if test="${tag<endP}">
                                            <li class="page-item">
                                                <a class="page-link" href="/Project_of_assignment/listP?pc=${pcc.id}&&index=${tag+1}">Next</a>
                                            </li>
                                        </c:if>
                                    </ul>
                                </nav>
